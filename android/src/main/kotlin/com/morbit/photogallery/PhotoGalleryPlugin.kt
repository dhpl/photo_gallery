    ...
    private fun getImageCursor(
        ...
        this.context.run {
            ...
            if (Build.VERSION.SDK_INT > Build.VERSION_CODES.Q) {
                ...
                if (skip != null && take != null && imageCursor != null) {
                    // Move cursor to the skip position
                    if (!imageCursor.moveToPosition(skip)) {
                        imageCursor.close()
                        return null
                    }
                    ...
                    do {
                        val row = Array(projection.size) { idx ->
                            when (imageCursor.getType(idx)) {
                                Cursor.FIELD_TYPE_INTEGER -> imageCursor.getLong(idx)
                                Cursor.FIELD_TYPE_FLOAT -> imageCursor.getFloat(idx)
                                Cursor.FIELD_TYPE_STRING -> imageCursor.getString(idx)
                                Cursor.FIELD_TYPE_BLOB -> imageCursor.getBlob(idx)
                                else -> null
                            }
                        }
                        matrixCursor.addRow(row)
                        count++
                    } while (imageCursor.moveToNext() && count < take)
